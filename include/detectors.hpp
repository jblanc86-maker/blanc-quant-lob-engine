...
public:
  ...
  void on_burst(double ms){ if(ms>burst_ms_) burst_ms_=ms; }
  void on_skew_ppm(double ppm){ skew_ppm_=ppm; }
  void inject_ppm(double gap,double corr,double skew,double burst){ s_gap_=gap; s_corr_=corr; s_skew_=skew; s_burst_=burst; }
  DetectorReadings readings() const{
    auto ppm=[&](uint64_t part){ return total_? (double(part)*1'000'000.0/double(total_)) : 0.0; };

...
#include <sstream>
#include <ctime>
namespace fs = std::filesystem; namespace lob {
bool ensure_dir(const std::string& p){ std::error_code ec; if(fs::exists(p,ec)) return true; return fs::create_directories(p,ec); }
static void esc(std::ostream& o,const std::string& s){ for(char c: s){ if(c=='"'||c=='\\') o<<'\\'<<c; else if(c=='\n') o<<"\\n"; else o<<c; } }
bool write_jsonl(const std::string& path,const TelemetrySnapshot& t){
  std::ofstream f(path,std::ios::app); if(!f) return false;
  f<<'{'<<"\"ts\":\""<<now_iso8601()<<"\","
    <<"\"input\":\""; esc(f,t.input_path); f<<"\","
...
std::string now_iso8601(){ std::time_t t=std::time(nullptr); std::tm tm{}; localtime_r(&t,&tm);
  std::ostringstream o; o<<std::put_time(&tm,"%Y-%m-%dT%H:%M:%S%z"); return o.str(); }
} // namespace lob

...
run_batch() {
  ...
  # Extract metrics from stdout using bash regex (no python3 dependency)
  local elapsed samples digest p99 p999 p9999 v999 v9999
  [[ "$line" =~ elapsed_ms=([0-9.]+) ]]    && elapsed="${BASH_REMATCH[1]}"   || elapsed=""
  [[ "$line" =~ samples=([0-9]+) ]]         && samples="${BASH_REMATCH[1]}"   || samples=""
  [[ "$line" =~ digest_fnv=(0x[0-9a-f]+) ]] && digest="${BASH_REMATCH[1]}"   || digest=""
  [[ "$line" =~ [[:space:]]p99=([0-9.eE+-]+)ms ]]    && p99="${BASH_REMATCH[1]}"    || p99=""
  [[ "$line" =~ [[:space:]]p99\.9=([0-9.eE+-]+)ms ]]  && p999="${BASH_REMATCH[1]}"  || p999=""
  [[ "$line" =~ [[:space:]]p99\.99=([0-9.eE+-]+)ms ]] && p9999="${BASH_REMATCH[1]}" || p9999=""
  [[ "$line" =~ p99\.9_valid=(true|false) ]]  && v999="${BASH_REMATCH[1]}"   || v999=""
  [[ "$line" =~ p99\.99_valid=(true|false) ]] && v9999="${BASH_REMATCH[1]}"  || v9999=""

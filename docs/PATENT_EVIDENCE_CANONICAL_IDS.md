...
### Versioning (schema tag)

Prefix the aggregate with a version label, e.g., `CANON_V1`, to distinguish digest definitions across future evolutions:
...
### 2.4 System Architecture (ASCII Overview)

```text
+-------------------------------+     +----------------------+     +---------------------+
| Ingress: Market Data          | --> | Order Book per Symbol| --> | Per-Symbol Digest  |
| - ITCH/FIX/Proprietary        |     | - Adds/Executes/Cancels|  | - Rolling hash over |
| - Replay/Live                 |     | - Journaled operations |  |   operations        |
+-------------------------------+     +----------------------+     +---------------------+
                            |                                   |                         |
                            v                                   v                         v
+-------------------------------+     +----------------------+     +---------------------+
| Canonical ID Module           | --> | Shard Emit (Map)     | --> | Shard Outputs       |
| - Normalize symbol bytes      |     | - (id, per_symbol)   |     | - Vector of pairs   |
| - Hash to 64-bit id           |     | - Stable per-shard   |     |   (id, digest)      |
+-------------------------------+     +----------------------+     +---------------------+
                            |                                                             |
                            v                                                             v
+---------------------------------------------------------------------------------------+
| Merge & Fold (Reduce)                                                                 |
| - Union/Sort by canonical id (tiebreak on raw symbol bytes)                           |
| - Iterate and mix (id, per_symbol_digest, schema_version) into aggregate root digest  |
| - Output: root_digest (CANON_V1)                                                      |
+---------------------------------------------------------------------------------------+
                            |
                            v
+-------------------------------+     +----------------------+     +---------------------+
| Telemetry & Provenance        | --> | CI Evidence Workflow | --> | Artifacts & Hashes |
| - Env/Compiler/Commit/Time    |     | - Cross-platform runs |     | - Logs/CSV/JSON    |
| - Doc hashes                  |     | - Archive outputs     |     | - SHA-256 checksums|
+-------------------------------+     +----------------------+     +---------------------+
```

Key contracts:

- Inputs: normalized symbol bytes, event stream per symbol, schema version tag.
- Outputs: per-symbol digest list per shard; root aggregate digest (CANON_V1);
  provenance artifacts (commit/tag/environment/doc hashes).
- Error modes: missing normalization, inconsistent shard tiebreak policy,
  version mismatch; all guarded by deterministic canonical ordering and version tags.

### 2.3 Symbol Canonicalization Boundary (Definition)

To ensure padding/encoding/locale insensitivity, the system must define the canonical symbol representation boundary prior to hashing.

- Canonical representation (one embodiment): fixed-width 8 bytes, right-padded with spaces, ASCII-only, uppercased.
- Alternative embodiment: variable-length normalized string (trim whitespace, upper-case, normalize Unicode), then pad/truncate to 8/16 bytes as required.
- Exchange-specific mapping: apply exchange symbology normalization rules (e.g., map micro/macro symbol variants) before hashing.

Pseudo-code (canonicalization + hashing):

```cpp
std::array<uint8_t, 8> canonicalize_symbol(std::string_view sym) {
        std::string s = normalize_encoding_to_ascii(sym);   // or UTF-8 normalization, as embodied
        to_upper_inplace(s);
        trim_inplace(s);
        std::array<uint8_t, 8> out{};
        // Right-pad with spaces to fixed width
        size_t n = std::min<size_t>(s.size(), out.size());
        std::memcpy(out.data(), s.data(), n);
        for (size_t i = n; i < out.size(); ++i) out[i] = ' ';
        return out;
}

uint64_t compute_canonical_id(std::string_view sym) {
        auto canon = canonicalize_symbol(sym);
        return fnv1a64(canon.data(), canon.size());         // or xxHash64, SipHash, Murmur, etc.
}
```

Note: If upstream already enforces a consistent fixed-width representation, the hashing embodiment MAY directly consume the upstream bytes; otherwise, canonicalization SHOULD be applied as above.

---

## 3. Alternative Embodiments (Platform Method)

This invention is a platform method, not a single implementation. Embodiments include:

- Stable hash functions: FNV-1a, xxHash64, CityHash, Murmur3, SipHash (keyed), SHA-256 truncated, BLAKE3 truncated.
- Symbol width variants: 4/8/16-byte fixed width; variable-length normalized; fixed-length padded; uppercase canonicalization; exchange-specific mapping.
- Ordering variants: sort IDs via std::vector + std::sort, radix sort, stable merge of per-shard sorted lists, B-tree index structures.
- Aggregate digest variants: mix (id || per_symbol_digest || symbol_len || schema_version), Merkle-tree aggregate, two-level rolling hash, commutative set-hash (order-insensitive, then proven equivalent via canonical ordering).

Each embodiment preserves the core property: aggregate digest invariance for the same symbol set and operations, independent of processing order and shard count.

---

## 4. Deterministic Shard Merge Specification

Map/Reduce pseudo-code for distributed operation:

```cpp
// Map stage (per shard)
vector<pair<uint64_t /*canonical_id*/, uint64_t /*per_symbol_digest*/>> shard_emit(const Events& shard_events) {
        unordered_map<uint64_t, uint64_t> per_symbol;
        for (const auto& ev : shard_events) {
                auto id = compute_canonical_id(ev.symbol);
                per_symbol[id] = fold_event(per_symbol[id], ev); // rolling digest
        }
        vector<pair<uint64_t, uint64_t>> out;
        out.reserve(per_symbol.size());
        for (auto& [id, dig] : per_symbol) out.emplace_back(id, dig);
        sort(out.begin(), out.end(), [](auto& a, auto& b) {
                if (a.first != b.first) return a.first < b.first;
                // deterministic tiebreak when collisions: compare symbol strings
                return symbol_of(a.first) < symbol_of(b.first);
        });
        return out;
}

// Reduce stage (global)
uint64_t aggregate_digest(const vector<vector<pair<uint64_t,uint64_t>>>& shard_pairs) {
        vector<pair<uint64_t,uint64_t>> all;
        for (auto& vec : shard_pairs) all.insert(all.end(), vec.begin(), vec.end());
        stable_sort(all.begin(), all.end(), [](auto& a, auto& b){ return a.first < b.first; });
        uint64_t agg = seed();
        for (auto& [id, dig] : all) agg = mix(agg, id, dig);
        return agg;
}
```

Proof sketch: The union of per-shard (id,digest) pairs over a fixed symbol set is independent of event ordering and shard partitioning. Canonical ordering (by id, with deterministic tiebreak) ensures a single deterministic fold order.

Compliance framing: This provides deterministic verification primitives used within broader compliance controls; compliance outcomes depend on organizational processes and regulators’ requirements.

### Figure: Distributed shards → pairs → global fold

![Shard Merge Flow](./images/shard_merge_flow.svg)

### ASCII Pipeline (Map → Reduce)
...
Example ordering stability:
- If ids collide: compare raw_symbol_bytes as tiebreak
- If shard count changes: union is identical (same (id,dig) set)
- If processing order changes: per-symbol digests stable; sort normalizes order
```

---

## 5. Cross-Platform Determinism Evidence

Platforms and compilers (to be expanded via CI):

- macOS arm64
  - `uname -a`: Darwin Mac 25.2.0 Darwin Kernel Version 25.2.0: Tue Nov 18 21:07:05 PST 2025; root:xnu-12377.61.12~1/RELEASE_ARM64_T6020 arm64
  - `sw_vers`: ProductVersion 26.2 (Build 25C56)
  - `clang --version`: Homebrew clang 21.1.5 (arm64-apple-darwin25)

- Linux x86_64 (planned)
  - GCC/Clang versions: capture via `gcc --version` / `clang --version`
  - Command line: record build/run commands
  - Evidence logs: store stdout/stderr, compute SHA-256

- ARM64 Linux (optional)
  - Compiler versions: as above
  - Evidence logs + hashes

Evidence log hashing (example):

```bash
shasum -a 256 artifacts/scaling/bench.csv > artifacts/scaling/bench.csv.sha256
shasum -a 256 artifacts/scaling/bench.png > artifacts/scaling/bench.png.sha256
```

Digest equality must match across platforms; discrepancies trigger investigation.
...
- Evidence doc SHA-256 (root): 8af97483e9ae57c5f0808a774e609b6e6268f7e96d8b4145356d452cb5772adb
...
- Machine info: Darwin Mac 25.2.0 (arm64)
...
Priority 1 (before filing):

- Correct filing status language ("not yet filed" until filed)
- Add collision detection + deterministic resolution policy
- Add canonicalization boundary definition (what is a Symbol?)
- Add alternative embodiments (hash, width, ordering, aggregate method)
- Replace any "cryptographic-quality" phrasing unless actually cryptographic
- Add provenance (git SHA/tag + hashes of doc/logs)

Priority 2 (high value, quick):

- Add Linux x86_64 run + log hash
- Add one ARM run + log hash
- Add distributed pseudo-code + merge proof sketch
- Add at least one figure: distributed shards → pairs → global fold

Priority 3 (strengthens defensibility):

- Expand tests: fuzz symbols (padding/case/encoding), large symbol counts (10k), randomized shard counts
- Add negative tests: demonstrate prior-art failure modes empirically
- Add performance note: overhead of ID computation + sorting vs baseline
...
Canonical IDs have high uniqueness (low collision probability)
...
Canonical IDs are deterministic and stable across multiple computations
...
Adding new symbols does not change the aggregate digest of an existing subset, unless that subset changes.

### Definition (set semantics)

The aggregate digest is computed over a declared set S of symbols. If S expands to S′ = S ∪ Δ, the digest over S remains unchanged; the digest over S′ is deterministic and invariant across order/shard changes.
...
When canonical IDs collide, ordering remains deterministic via a compound tiebreak.

### Default policy (embodiment)

Sort pairs by (canonical_id, raw_symbol_bytes). This preserves canonical-ID ordering and resolves collisions without rehash.
...
Preliminary / Not Exhaustive (references to be verified):

- Deterministic order book processing approaches (single-symbol focus)
- Sharding-based routing using symbol hash (load balancing, not verification)
- Cross-datacenter snapshot comparison requiring identical processing order

We will update this section with verified patent/publication identifiers and links after a professional prior art search.
